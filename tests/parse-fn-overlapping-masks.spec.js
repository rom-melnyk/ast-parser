...
describe('parse() nodes with overlapping masks', () => {
    it('should parse correctly if mask from different nodes overlap each other', () => {
        ...
        assert.equal(parsed1.length, 5, 'nodes count (1)');
        assert.equal(parsed2.length, 5, 'nodes count (2)');

        assert.equal(parsed1[2].classId, '+', '"+" recognition');
        assert.equal(parsed2[2].classId, '++', '"++" recognition');
    });
});
